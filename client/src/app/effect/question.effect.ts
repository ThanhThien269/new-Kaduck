...
export class QuestionEffects{
  ...
  getQuestions$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap((action) => this.questionService.getQuestions().pipe(map((questions) => {
  ...
  getQuestion$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap((action) => this.questionService.getQuestion(action.id).pipe(map((question) => {
      return QuestionActions.getQuestionSuccess({question:question})
    }), catchError((error) => of(QuestionActions.getQuestionFailure({error:error})))))
  ))

  postQuestion$ = createEffect(() => this.actions$.pipe(
    ofType(QuestionActions.postQuestion),
    switchMap((action) => this.questionService.postQuestion(action.question).pipe(map((question) => {
      return QuestionActions.postQuestionSuccess({question:question})
    }), catchError((error) => of(QuestionActions.postQuestionFailure({error:error})))))
  ))

  updateQuestion$ = createEffect(() => this.actions$.pipe(
    ofType(QuestionActions.updateQuestion),
    switchMap((action) => this.questionService.updateQuestion(action.question).pipe(map((question) => {
      return QuestionActions.updateQuestionSuccess({question:question})
  ...
  deleteQuestion$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap((action) => this.questionService.deleteQuestion(action.question).pipe(map((question) => {
      return QuestionActions.deleteQuestionSuccess({question:question})
